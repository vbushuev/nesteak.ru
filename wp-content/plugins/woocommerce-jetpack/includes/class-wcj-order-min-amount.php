		...
		if ( $cart_total < $minimum ) {
			if ( is_cart() ) {
				if ( 'yes' === get_option( 'wcj_order_minimum_amount_cart_notice_enabled', 'no' ) ) {
					...
					$notice_function(
						sprintf( apply_filters( 'booster_option', 'Минимальная сумма заказа %s Выберете еще товаров на сумму %s', get_option( 'wcj_order_minimum_amount_cart_notice_message' ) ),
							wc_price( $minimum ),
				...
				wc_add_notice(
					sprintf( apply_filters( 'booster_option', 'Минимальная сумма заказа %s Выберете еще товаров на сумму %s', get_option( 'wcj_order_minimum_amount_error_message' ) ),
						wc_price( $minimum ),
						wc_price( $minimun - $cart_total )
					),
					'error'
				);
			}
