...
if ( ! class_exists( 'Vffront_Plugin_Install' ) ) :
	/**
	 * The Vffront plugin install class
	 */
	class Vffront_Plugin_Install {
		...
		public function plugin_install_scripts( $hook_suffix ) {
			...
			wp_enqueue_script( 'vffront-plugin-install', get_template_directory_uri() . '/assets/js/admin/plugin-install' . $suffix . '.js', array( 'jquery', 'updates' ), $vffront_version, 'all' );

			wp_enqueue_style( 'vffront-plugin-install', get_template_directory_uri() . '/assets/css/admin/plugin-install.css', array(), $vffront_version, 'all' );
		}
			...
			if ( current_user_can( 'install_plugins' ) && current_user_can( 'activate_plugins' ) ) {
				if ( is_plugin_active( $plugin_slug . '/' . $plugin_file ) ) {
					...
					$button = array(
						...
						'url'     => '#',
						'classes' => array( 'vffront-button', 'disabled' ),
					);
					...
					$button = array(
						...
						'url'     => $url,
						'classes' => array( 'vffront-button', 'activate-now' ),
					);
					...
					$button = array(
						...
						'url'     => $url,
						'classes' => array( 'vffront-button', 'sf-install-now', 'install-now', 'install-' . $plugin_slug ),
					);
...
return new Vffront_Plugin_Install();
